...
//                    }
     ...
     */
    public function initiateGWPayment(array $requestData)
    {
        ...
        return $this->callToGWApi($this->data, $header, $this->config['connect_from_localhost']);

         // Here we will define the response pattern
        // $formattedResponse = $this->formatResponse($response); // Here we will define the response pattern

        // if ($type == 'hosted') {
        //     if (isset($formattedResponse['GatewayPageURL']) && $formattedResponse['GatewayPageURL'] != '') {
        //         $this->redirect($formattedResponse['GatewayPageURL']);
        //         // Redirect::to($formattedResponse['GatewayPageURL']);
        //     } else {
    ...
    public function initiateRefund(array $requestData)
    {
        $header = [];

        $this->setApiUrl($this->config['apiDomain'] . $this->config['apiUrl']['make_refund']);

        $requestdata['store_id'] = $this->getStoreId();
        $requestdata['store_passwd'] = $this->getStorePassword();

        // Now, call the Gateway API
        return $this->callToGWApi($requestData, $header, $this->config['connect_from_localhost']);
    }
    ...
    public function setProductInfo(array $info)
    {
        ...
        $this->data['product_category'] = (isset($info['product_category'])) ? $info['product_category'] : 'Goods'; // String (100)	Mandatory - Mention the product category. Example: Electronic or topup or bus ticket or air ticket
